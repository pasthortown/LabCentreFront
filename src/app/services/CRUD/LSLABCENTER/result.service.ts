...
export class ResultService {
   ...
   url = environment.api_lslabcenter + 'result/';
   ...
   post(result: Result): Promise<any> {
      return this.http.post(this.url, JSON.stringify(result), this.options).toPromise()
      .then( r => {
         return r;
      }).catch( error => { this.handledError(error); });
   }

   put(result: Result): Promise<any> {
      return this.http.put(this.url, JSON.stringify(result), this.options).toPromise()
      .then( r => {
         return r;
      }).catch( error => { this.handledError(error); });
   }
