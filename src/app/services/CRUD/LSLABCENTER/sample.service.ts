...
export class SampleService {
   ...
   url = environment.api_lslabcenter + 'sample/';
   ...
   post(sample: Sample): Promise<any> {
      return this.http.post(this.url, JSON.stringify(sample), this.options).toPromise()
      .then( r => {
         return r;
      }).catch( error => { this.handledError(error); });
   }

   put(sample: Sample): Promise<any> {
      return this.http.put(this.url, JSON.stringify(sample), this.options).toPromise()
      .then( r => {
         return r;
      }).catch( error => { this.handledError(error); });
   }
