...
export class LaboratoryService {
   ...
   url = environment.api_lslabcenter + 'laboratory/';
   ...
   post(laboratory: Laboratory): Promise<any> {
      return this.http.post(this.url, JSON.stringify(laboratory), this.options).toPromise()
      .then( r => {
         return r;
      }).catch( error => { this.handledError(error); });
   }

   put(laboratory: Laboratory): Promise<any> {
      return this.http.put(this.url, JSON.stringify(laboratory), this.options).toPromise()
      .then( r => {
         return r;
      }).catch( error => { this.handledError(error); });
   }
