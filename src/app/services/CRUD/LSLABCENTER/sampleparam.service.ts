...
export class SampleParamService {

   url = environment.api_lslabcenter + 'sampleparam/';
   ...
   post(sampleparam: SampleParam): Promise<any> {
      return this.http.post(this.url, JSON.stringify(sampleparam), this.options).toPromise()
      .then( r => {
         return r;
      }).catch( error => { this.handledError(error); });
   }

   put(sampleparam: SampleParam): Promise<any> {
      return this.http.put(this.url, JSON.stringify(sampleparam), this.options).toPromise()
      .then( r => {
         return r;
      }).catch( error => { this.handledError(error); });
   }
