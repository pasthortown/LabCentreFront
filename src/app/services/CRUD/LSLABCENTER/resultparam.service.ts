...
export class ResultParamService {
   ...
   url = environment.api_lslabcenter + 'resultparam/';
   ...
   post(resultparam: ResultParam): Promise<any> {
      return this.http.post(this.url, JSON.stringify(resultparam), this.options).toPromise()
      .then( r => {
         return r;
      }).catch( error => { this.handledError(error); });
   }

   put(resultparam: ResultParam): Promise<any> {
      return this.http.put(this.url, JSON.stringify(resultparam), this.options).toPromise()
      .then( r => {
         return r;
      }).catch( error => { this.handledError(error); });
   }
