...
export class ProfileService {
   ...
   url = environment.api_lslabcenter + 'profile/';
   ...
   post(profile: Profile): Promise<any> {
      return this.http.post(this.url, JSON.stringify(profile), this.options).toPromise()
      .then( r => {
         return r;
      }).catch( error => { this.handledError(error); });
   }

   put(profile: Profile): Promise<any> {
      return this.http.put(this.url, JSON.stringify(profile), this.options).toPromise()
      .then( r => {
         return r;
      }).catch( error => { this.handledError(error); });
   }
